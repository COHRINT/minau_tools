...
def plot_error(x_truth, x_hat, P, num_ownship_states,asset_id, avg_latency, event_pubs, implicit_cnt, explicit_cnt, num_rows, num_cols, asset_idx, asset_name, last_asset):
    ...
    if num_assets == 1 and num_ownship_states == 1:
        ...
        for i in range(num_assets):
            for j in range(num_ownship_states):
                ...
                plt.subplot(num_rows,num_cols,cnt + 4*asset_idx)
                ...
                if last_asset:
                    plt.xlabel("seconds")
                if asset_idx == 0:      #Only print top titles on first one
...
parser.add_argument("-a","--asset",type=str,help="Name of asset you want to analyze(ex:dory)",required=True)
...
topic_prefix = "/" + chosen_asset + "/etddf/estimate/"
topic_prefix_length = len(topic_prefix)

for topic in topics:
    if topic[:topic_prefix_length] == topic_prefix and topic != topic_prefix + "network":
        asset_list.append(topic[topic_prefix_length:])

NUM_ROWS = len(asset_list)          # Rows is number of assets + 1 for odom
NUM_COLS = 4                        # Cols for graph, x, y, x_dot, y_dot

for asset_idx in range(len(asset_list)):
    if asset_idx == 0:
        use_etddf = False
    else:
        use_etddf = True
    
    if asset_idx == len(asset_list) - 1:
        last_asset = True
    else:
        last_asset = False

    MEASURED_NAME = asset_list[asset_idx]
    ...
    plot_error(x_truth_bag, x_hat_bag0, p_bag0, 8,0, avg_latency, event_pubs, implicit_cnt, explicit_cnt, NUM_ROWS, NUM_COLS, asset_idx,MEASURED_NAME,last_asset)
    # Get erros
...
plt.subplots_adjust(hspace=0.5,wspace=0.3, left=0.05, right=0.95, bottom=0.05, top = .92)
fig = plt.gcf()
